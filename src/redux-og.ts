...
type TodoActionTypes =   CreateTodoActionType | EditTodoActionType | DeleteTodoActionType;
...
const todosReducer = (state: Todo[]=todosInitialState,action:TodoActionTypes)=>{
        ...
        switch(action.type){
            ...
            case EDIT_TODO:
                return state.map(value=>  value.id === action.payload.id  ?  {...value,desc:action.payload.desc} : value)
